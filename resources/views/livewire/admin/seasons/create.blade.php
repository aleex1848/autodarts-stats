...
new class extends Component {
    ...
    public function save(): void
    {
        ...
        $imagePath = null;
        if ($this->image) {
            $imagePath = $this->image->store('season-images', 'public');
        }
...
<section class="w-full space-y-6">
    ...
    <form wire:submit="save" class="space-y-6">
        ...
        <div class="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
            <flux:heading size="lg" class="mb-4">{{ __('Bild') }}</flux:heading>

            <div>
                <flux:file-upload wire:model="image" :label="__('Saison-Bild (optional)')">
                    <flux:file-upload.dropzone 
                        heading="{{ __('Bild hochladen') }}" 
                ...
                @if ($image)
                    <div class="mt-3">
                        ...
                        >
                            <x-slot name="actions">
                                <flux:file-item.remove wire:click="$set('image', null)" aria-label="{{ __('Bild entfernen') }}" />
                            </x-slot>
                        </flux:file-item>
                    </div>
                @endif
