...
new class extends Component {
    ...
    {
        return [
            'registrations' => $this->season->registrations,
            ...
            'players' => Player::query()
                ...
                ->whereNotIn('id', $this->season->participants->pluck('player_id'))
                ->orderBy('name')
                ->limit(20)
                ->get(),
    ...
    {
        $registration = SeasonRegistration::findOrFail($registrationId);
        $registration->update(['status' => RegistrationStatus::Confirmed->value]);

        // Create participant
        SeasonParticipant::firstOrCreate([
    ...
    {
        $participant = SeasonParticipant::findOrFail($participantId);
        $participant->delete();
    ...
    {
        if ($this->season->participants->count() < 2) {
            $this->dispatch('notify', title: __('Mindestens 2 Teilnehmer benötigt'), variant: 'error');
            
            return;
        }

        app(LeagueScheduler::class)->generateMatchdays($this->season, $this->season->participants);

        $this->season->update(['status' => LeagueStatus::Active->value]);
    ...
    public function splitSeason(): void
    {
        $participantCount = $this->season->participants->count();
        ...
        $participants = $this->season->participants->shuffle();
        ...
        for ($i = 0; $i < $this->numberOfSplits; $i++) {
            $splits[] = [
                'player_ids' => $participants->skip($i * $playersPerSplit)
                    ...
                    ->pluck('player_id')
                    ->toArray(),
                'max_players' => $this->season->max_players,
            ];
        }

        app(SplitSeason::class)->handle($this->season, $splits);

        $this->showSplitModal = false;
        $this->dispatch('notify', title: __('Saison wurde aufgeteilt'));
...
<section class="w-full space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-4">
        <div>
            ...
            <flux:subheading>
                {{ $season->description ?? __('Saison-Details und Verwaltung') }}
                @if ($season->league)
                    - {{ $season->league->name }}
                @endif
            </flux:subheading>
        ...
        <div class="flex gap-2">
            ...
            <flux:button
                variant="ghost"
                :href="route('admin.leagues.show', $season->league)"
                wire:navigate
            >
                {{ __('Zurück zur Liga') }}
            </flux:button>
    ...
    <div class="flex gap-2 border-b border-zinc-200 dark:border-zinc-700">
        ...
        >
            {{ __('Anmeldungen') }} ({{ $season->registrations->count() }})
        </button>
        ...
        >
            {{ __('Teilnehmer') }} ({{ $season->participants->count() }})
        </button>
        ...
        >
            {{ __('Spieltage') }} ({{ $season->matchdays->count() }})
        </button>
        ...
        <div class="space-y-6">
            <div class="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
                ...
                <dl class="grid gap-4 md:grid-cols-2">
                    ...
                    <div>
                        <dt class="text-sm font-medium text-zinc-500 dark:text-zinc-400">{{ __('Liga') }}</dt>
                        <dd class="mt-1 text-sm text-zinc-900 dark:text-zinc-100">
                            <a href="{{ route('admin.leagues.show', $season->league) }}" class="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300">
                                {{ $season->league->name }}
                            </a>
                        </dd>
                    </div>
                    ...
                    <div>
                        ...
                        <dd class="mt-1 text-sm text-zinc-900 dark:text-zinc-100">{{ $season->max_players }}</dd>
                    </div>
                    ...
                    <div>
                        ...
                        <dd class="mt-1 text-sm text-zinc-900 dark:text-zinc-100">{{ $season->days_per_matchday }}</dd>
                    </div>
                    ...
                    @if ($season->registration_deadline)
                        <div>
                            ...
                            <dd class="mt-1 text-sm text-zinc-900 dark:text-zinc-100">
                                {{ $season->registration_deadline->format('d.m.Y H:i') }}
                            </dd>
                        </div>
                    @endif
                </dl>
            ...
            @if ($season->status === 'registration' && $season->participants->count() >= 2)
                <div class="flex flex-wrap gap-2">
                    <flux:button wire:click="generateMatchdays" variant="primary" icon="calendar">
                        {{ __('Spieltage generieren') }}
                    </flux:button>

                    @if ($season->participants->count() > $season->max_players)
            ...
            @if ($season->participants->count() > $season->max_players)
                <flux:callout variant="warning" icon="exclamation-triangle">
                    {{ __('Die Saison hat mehr Teilnehmer (:count) als die maximale Spielerzahl (:max). Erwäge die Saison aufzuteilen.', [
                        'count' => $season->participants->count(),
                        'max' => $season->max_players
                    ]) }}
                </flux:callout>
            @endif
        </div>
    ...
    @if ($activeTab === 'registrations')
        <div class="space-y-4">
            @if ($season->registrations->where('status', 'pending')->count() > 0)
    ...
    <flux:modal wire:model="showSplitModal" class="space-y-6">
        <div>
            <flux:heading size="lg">{{ __('Saison aufteilen') }}</flux:heading>
            <flux:subheading>
                {{ __('Teile die :count Teilnehmer auf mehrere Saisons auf', ['count' => $season->participants->count()]) }}
            </flux:subheading>
        </div>

        <form wire:submit="splitSeason" class="space-y-4">
            <flux:input
                wire:model="numberOfSplits"
                :label="__('Anzahl der Saisons')"
                type="number"
                min="2"
                :max="$season->participants->count()"
                required
            />

            <p class="text-sm text-zinc-600 dark:text-zinc-400">
                {{ __('Dies wird :count Saisons mit je ca. :players Spielern erstellen.', [
                    'count' => $numberOfSplits,
                    'players' => floor($season->participants->count() / max($numberOfSplits, 1))
                ]) }}
            </p>
