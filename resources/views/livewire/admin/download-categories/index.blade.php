...
new class extends Component {
    ...
    {
        return [
            'categories' => DownloadCategory::query()->withCount('downloads')->orderBy('name')->get(),
        ];
    }
    ...
    protected function rules(): array
    {
        return [
            ...
            'slug' => [
                ...
                'string',
                'max:255',
                Rule::unique(DownloadCategory::class, 'slug')->ignore($this->editingCategoryId),
            ],
    ...
    {
        $category = DownloadCategory::findOrFail($categoryId);
    ...
    public function saveCategory(): void
    {
        ...
        if ($this->editingCategoryId) {
            $category = DownloadCategory::findOrFail($this->editingCategoryId);
    ...
    {
        $category = DownloadCategory::withCount('downloads')->findOrFail($categoryId);

        if ($category->downloads_count > 0) {
            $this->addError('delete', __('Diese Kategorie kann nicht gelöscht werden, da sie noch Downloads enthält.'));
    ...
    public function deleteCategory(): void
    {
        if ($this->categoryIdBeingDeleted) {
            $category = DownloadCategory::findOrFail($this->categoryIdBeingDeleted);
            $category->delete();
        }
    ...
    {
        $this->reset('editingCategoryId', 'name', 'slug', 'description');
    }
}; ?>
...
<section class="w-full space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-4">
        <div>
            <flux:heading size="xl">{{ __('Download-Kategorien') }}</flux:heading>
            <flux:subheading>{{ __('Verwalte Download-Kategorien') }}</flux:subheading>
        </div>
            ...
            <tbody class="divide-y divide-zinc-200 dark:divide-zinc-800">
                @forelse ($categories as $category)
                    <tr wire:key="category-{{ $category->id }}">
                        ...
                        <td class="px-4 py-3 text-sm text-zinc-600 dark:text-zinc-300">
                            <flux:badge size="sm" variant="subtle">{{ $category->downloads_count }}</flux:badge>
                        </td>
                        <td class="px-4 py-3 text-right text-sm">
                            <div class="flex justify-end gap-2">
                                ...
                                <flux:button
                                    ...
                                    :disabled="$category->downloads_count > 0"
                                >
                                    {{ __('Löschen') }}
                                </flux:button>
                            </div>
    ...
    <flux:modal wire:model="showCategoryFormModal" class="space-y-6">
        <div>
            ...
            <flux:subheading>
                {{ __('Pflege Name, Slug und Beschreibung der Kategorie') }}
            </flux:subheading>
        </div>
